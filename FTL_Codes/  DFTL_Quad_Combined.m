%%% NOTE: Quad is just the name. It can be any function f(S_damping)

%%%% SOLVES: 1. The final Velocities are actually converging to zero
%%%%         2. System behaves moderately. Neighter highly damped nor under
...
for t= 2:totalTimeFrames
    ...
    for i = 2 : size
        ...
        if i==size
            X_mat(i,t) = X_mat(i,t-1) + delta_time*(VelX_mat(i,t-1)) - s_new * corr_X(size-1)/2;
            Y_mat(i,t) = Y_mat(i,t-1) + delta_time*(VelY_mat(i,t-1))  + (delta_time * delta_time) * (-g) - s_new * corr_Y(size-1)/2;
            
        % Adding both the forward and backward constraints            
        else
            X_mat(i,t) = X_mat(i,t-1) + delta_time*(VelX_mat(i,t-1)) - s_new * (corr_X(i-1)/2 + corr_X(i)/2);
            Y_mat(i,t) = Y_mat(i,t-1) + delta_time*(VelY_mat(i,t-1))  + (delta_time * delta_time) * (-g) - s_new * (corr_Y(i-1)/2 + corr_Y(i)/2);            
        end 
...
%    s_damp = 1 - ((t)^2/(totalTimeFrames^2));
    s_new = 0.8 - 0.1 * ((t)^2/(totalTimeFrames^2));
...
for t = 1: 1: totalTimeFrames
    ...
    for i = 1 : size-1
     ...
     if count < 10  
%      s = strcat('C:\Users\Lenovo\Downloads\FTL project\Images_DFTL_Quad_Combined\test-00',int2str(count));
       s = strcat('/home/vedant/Downloads/FTL-project/Images_DFTL_Quad_Combined/test-00',int2str(count));
     elseif count < 100  
%       s = strcat('C:\Users\Lenovo\Downloads\FTL project\Images_DFTL_Quad_Combined\test-0',int2str(count));      
       s = strcat('/home/vedant/Downloads/FTL-project/Images_DFTL_Quad_Combined/test-0',int2str(count));

     else   
%         s = strcat('C:\Users\Lenovo\Downloads\FTL project\Images_DFTL_Quad_Combined\test-',int2str(count));        
       s = strcat('/home/vedant/Downloads/FTL-project/Images_DFTL_Quad_Combined/test-',int2str(count));

...
for t= 2:totalTimeFrames
    ...
    for i = 2 : size
        ...
        if i==size
            X_mat(i,t) = X_mat(i,t-1) + delta_time*(VelX_mat(i,t-1)) - s_damp * corr_X(size-1)/2;
            Y_mat(i,t) = Y_mat(i,t-1) + delta_time*(VelY_mat(i,t-1))  + (delta_time * delta_time) * (-g) - s_damp * corr_Y(size-1)/2;
            
        % Adding both the forward and backward constraints            
        else
            X_mat(i,t) = X_mat(i,t-1) + delta_time*(VelX_mat(i,t-1)) - s_damp * (corr_X(i-1)/2 + corr_X(i)/2);
            Y_mat(i,t) = Y_mat(i,t-1) + delta_time*(VelY_mat(i,t-1))  + (delta_time * delta_time) * (-g) - s_damp * (corr_Y(i-1)/2 + corr_Y(i)/2);            
        end 
